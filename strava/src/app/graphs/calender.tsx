...
type Event = [
  ...
  // Calendar component
  export default function EventsCalender({ userActivities }: { userActivities: any[] }) {

    const [modalIsOpen, setIsOpen] = useState(false);
    const [content, setContent] = useState<React.JSX.Element[] | String>()
    ...
    const handleClick = (day: string, eventText: React.JSX.Element[]| String) => {
      openModal()
      setContent(eventText)
      console.log(day, eventText)
    }
    ...
    for (let activity of lastMonth) {
      ...
      if (dateFound) {
        const watts : number | null = activity["average_watts"] ? activity["average_watts"] : null;
    ...
    const finalHtml = allDates.map((date) => {
      ...
      if (date.events.length) {
        ...
        return (
          <div key={uuidv4()} className="flex flex-col bg-gray-100 rounded-lg justify-between p-4 min-h-[150px] cursor-pointer" onClick={() => handleClick(date.date, eventText)}>
            <div className="py-2">
        ...
        return (
          <div key={uuidv4()} className="p-4 bg-gray-100 rounded-lg p-2 min-h-[150px] cursor-pointer" onClick={() => handleClick(date.date, "rest day")}>
            <p className="font-semibold py-2 text-sm">{date.date}</p>
          ...
          <div className="w-auto inline-block">
            <div className="flex flex-col p-2">
              <p className="flex font-bold items-center py-2">
                ...
                <FontAwesomeIcon icon={faArrowRight} size="sm" className="mx-2" /> High.
              </p>
            </div>
